...
int CFastFourierTransform::BitReverse(int j)
{
	int j2;
	int j1 = j;
	int k = 0;
...
float* CFastFourierTransform::FFT2(float* pSample, size_t pSampleSize)
{
	// Assume n is a power of 2
	int n = pSampleSize;
	int nu = (int)(log((float)(n)) / log((float)(2)));
	int n2 = n >> 1;
	int nu1 = nu - 1;
	...
	for (int i = 0; i < (n >> 1); i++)		
		decibel[i] = 0.05 * log10((float)(sqrt((xre[i] * xre[i]) + (xim[i] * xim[i]))));
