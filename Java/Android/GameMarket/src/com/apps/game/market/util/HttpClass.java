...
public class HttpClass {
	...
	public EntityResponse request(EntityRequest req) {
		final Map<String, String> header = req.getHeader();
		final String webUrl = req.getUrl();
		final String body = req.getBody();
		final String charset = req.getCharset();
	...
	private EntityResponse getBody(HttpURLConnection connection, EntityRequest req) {
		final String charset = req.getCharset();
		final boolean string = req.getString();
				
		final EntityResponse resp = new EntityResponse();
		...
		final BufferedReader bufferReader = new BufferedReader(streamReader);
    ...
    private InputStream getInputStream(HttpURLConnection connection) {
		...
		if (contentEncoding != null) {
			if (contentEncoding.equals("gzip")) {
				try {
					final InputStream gzip = new GZIPInputStream(inputStream);
					return gzip;
				} catch (IOException e) {
					closeStream(inputStream);
					return null;
