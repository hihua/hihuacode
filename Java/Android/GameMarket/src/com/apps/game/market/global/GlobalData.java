...
public class GlobalData {
	...
	private final Map<String, EntityApp> remoteApps = new ConcurrentHashMap<String, EntityApp>();
	private final List<EntityAppInfo> localApps = new Vector<EntityAppInfo>();
	private final List<EntityApp> downloadApps = new Vector<EntityApp>();
	private final List<EntityApp> collectApps = new Vector<EntityApp>();
	private final List<EntityApp> browseApps = new Vector<EntityApp>();
	private final Map<Long, List<EntityAd>> ads = new HashMap<Long, List<EntityAd>>();
	private final List<EntityColumn> columns = new Vector<EntityColumn>();
	private final List<EntityTag> tags = new Vector<EntityTag>();
	...
	public boolean init(Context context) {
		final List<EntityAppInfo> list = ApkManager.getApps(context);
		if (list != null) {
	...
	public void putRemoteApp(EntityApp remoteApp) {
		final String packageName = remoteApp.getPackageName();
	...
	public void addLocalApp(EntityAppInfo entityAppInfo) {
		final String packageName = entityAppInfo.getPackageName();
	...
	public void addDownloadApp(EntityApp remoteApp) {
		final String packageName = remoteApp.getPackageName();
		for (EntityApp entityApp : downloadApps) {
			if (entityApp.getPackageName().equals(packageName))
				return;
		}
