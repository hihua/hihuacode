...
public class ScrollViewAd extends ViewGroup {	
	...
	private final Handler mHandle = new Handler() {
        ...
        public void handleMessage(Message msg) {
			toScroll(true, true);
			if (mCallBack != null) {
				final int total = getChildCount() - mFake;
				switch (total) {
					case 1:
					...
					case 2:
						final int screen = mScreen % total;
			...
			case MotionEvent.ACTION_UP: {				
				...
				if (velocityX > 600.0f)
					...
					if (velocityX < -600.0f)
						...
						if (mLastTime > now && mLastTime - now < 100)
							handle = true;
						else {
							if (mLastX > mTouchX)
