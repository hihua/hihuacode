...
public class RequestComment extends RequestBase {	
	...
	protected void onTask(EntityRequest req) {
		...
		if (resp != null) {
			List<EntityComment> list = parse(resp);
			resp.close();
			if (list != null) {
	...
	private List<EntityComment> parse(EntityResponse resp) {
		...
		Element parent = root.element("comments");
		if (parent == null)
			return null;
		...
		Iterator<Element> elements = parent.elementIterator("comment");
		if (elements != null) {
			List<EntityComment> list = new Vector<EntityComment>();
