...
public class RequestAppRecommend extends RequestApp {	
	private final int mResId = R.string.request_app_recommend;
	...
	public RequestAppRecommend() {
		
	}
	
	public RequestAppRecommend(long id) {
		super(id);
	}
	...
	public void request(long postion, long count) {
		if (mUU)
			mBody = String.format("&id=%d&p=%d&ps=%d", mId, postion, count);
		else
	...
	private void reqUU(EntityRequest req) {
		String url = setUrl(R.string.request_uu_app_column);
		url += mBody;
